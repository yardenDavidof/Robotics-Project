...
// predict by laser
double Particle::prodByScan(Location::Position* delta, double laser[], GridMap grid){
	float distance = sqrt(pow(delta->location->x * ConfigurationManager::getInstance()->getGridResolutionCM(), 2) + pow(delta->location->y * ConfigurationManager::getInstance()->getGridResolutionCM(), 2));
		position->setX(position->getX() + getXDelta(position->yaw, distance));
		position->setY(position->getY() + getYDelta(position->yaw, distance));

		position->yaw += delta->getYaw();
		int errors = 0;
